...
@Component({
  selector: 'app-top-scorer-stats-premier',
  templateUrl: './top-scorer-stats-premier.component.html',
  styleUrls: ['./top-scorer-stats-premier.component.css']
})
export class TopScorerStatsPremierComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.topScorerService.getAlltopScorerStatsPremier().subscribe((result)=>{
  ...
  checkPlayerPremier(term2:string) {
    this.topScorerService.getAlltopScorerStatsPremier().subscribe((result)=>{
      this.scorer=result.data;
